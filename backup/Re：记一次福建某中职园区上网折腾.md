...
由于前章（2023/11）写的太烂了所以直接Remake

## 准备工作以及注意事项

1. **脑子**
2. **不会帮你搭建 不接受教你搭建 多Bing多Google 不要使用Baidu 因为你所在的地区不太支持你这么做（类似于搭梯）**
3. 一台标准的x86或arm设备充当**服务端**，可运行Windows、Linux或OpenWRT（运行内存最好大于或等于512M，否则容易爆运存）。
4. OpenWRT设备[可选，充当透明代理，**客户端**]
5. **不建议任一端使用MT7621搭建，7621已经过时了。仅测试过Socks，200Mbps附近且CPU吃不满，重写前的测试环境为MT7621**。
6. **不建议任一端运存小于或等于512M，在IPQ6018、MT7981+256M+“PassWall”（自行猜测原意）环境下经常爆内存导致卡死杀进程**。
7. **tcping延迟可能会高一些。**

**最高可用速率取决于设备和上层交换机、网关性能，以及中转会给带宽打折。比如我手上的路由普遍只有1000Mbps，没有2.5G 。**

**我没有测试使用光纤交流数据的华为交换机的带宽上限是多少，也不知道一栋楼是不是共用一个外网千兆。**
...
点击添加后，类型选择Socks，填写监听端口，并确保不会被占用。

当然，你也可以在这里选择用Sing-Box后再选择Socks。

身份认证勾选随意，安全性。不勾你也不会失去什么，还没看见有人会去扫网段。

**如果有“局域网访问”，请务必记得勾选。**

**创建完服务后，在用户管理前给相关服务端配置的打勾，记得勾选服务器端的启用。**

在Windows/Linux环境下，使用Sing-Box可以搭建。[配置文件示例](https://raw.githubusercontent.com/LinMoyu233/Archives/refs/heads/main/socks-config.conf)
...
1. > https://www.coolapk.com/feed/51778131

3. 我现在才知道IP段50-57都是贵集团的...7条IP日常只见到50和52。57开放端口多一些其他开放端口都一样。
...
如果某些人看见：**把极域自带功能再拉出来说是新机房特点，那我真不知道该说什么了。甚至检测还是人工拉黑IP...**